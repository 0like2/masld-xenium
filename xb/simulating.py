...
def allcombs(adata):
    ...
    logs=[True,False]
    perc=0.1
    sil_scores = {} # Dictionary to store Silhouette scores
    for qv in qvs:
        for nuc in [1,0]:
            if nuc==1:
                                ...
                                for lgo in logs:
                                    for hvg in hvgs:
                                        ...
                                        if nv==0:
                                            ...
                                            print('DEFAULT')
                                            # norm -> normstatus, lg -> logstatus
                                            result,ttc=main_preprocessing(adata_f2,neigh=neigh,npc=npc,target_sum=target_sum,scale=scale,hvg=hvg,default=True,normstatus=nm,logstatus=lgo)
                                            ...
                                            # Calculate Silhouette Score (Default)
                                            try:
                                                if 'X_pca' in result.obsm.keys():
                                                    X_data = result.obsm['X_pca']
                                                else:
                                                    X_data = result.X
                                                
                                                labels = result.obs['louvain_1_4']
                                                
                                                # Subsample for performance if > 10k cells
                                                if X_data.shape[0] > 10000:
                                                    indices = np.random.choice(X_data.shape[0], 10000, replace=False)
                                                    X_sa = X_data[indices]
                                                    lab_sa = labels.iloc[indices]
                                                else:
                                                    X_sa = X_data
                                                    lab_sa = labels
                                                    
                                                # Handle sparse matrix
                                                if hasattr(X_sa, "toarray"):
                                                    X_sa = X_sa.toarray()
                                                    
                                                score = silhouette_score(X_sa, lab_sa)
                                            except Exception as e:
                                                print(f"Sil score error: {e}")
                                                score = -1.0
                                            
                                            sil_scores['DEFAULT_louv'] = score
                                        if nv==1:
                                            adata_f2=adata_f1.copy()
                                            print(nucdic[nuc]+'_qv'+str(qv)+'_neighs'+str(neigh)+'_npc'+str(npc)+'_ts'+str(target_sum)+'_hvg_'+str(hvg)+'_scale_'+str(scale))
                                            # norm -> normstatus, lg -> logstatus, total_clusters added
                                            result=main_preprocessing(adata_f2,neigh=neigh,npc=npc,target_sum=target_sum,scale=scale,hvg=hvg,total_clusters=ttc,normstatus=nm,logstatus=lgo)
                                            sumresult=result.obs.loc[:,['leiden_1_4','louvain_1_4']]
                                            ...
                                            # Calculate Silhouette Score (Variations)
                                            try:
                                                if 'X_pca' in result.obsm.keys():
                                                    X_data = result.obsm['X_pca']
                                                else:
                                            ...
                                            col_name = nucdic[nuc]+'_qv'+str(qv)+'_ng'+str(neigh)+'_pc'+str(npc)+'_ts'+str(target_sum)+'_hvg_'+str(hvg)+'_scale_'+str(scale)+'_norm_'+str(nm)+'_log_'+str(lgo)+'_louv'
                                            sil_scores[col_name] = score

    return allres, sil_scores
...
def main_preprocessing(adata,target_sum=100,mincounts=10,mingenes=3,neigh=15,npc=0,nuc=1,scale=False,hvg=False,default=False,total_clusters=30,default_resol=1.6,logstatus=True,normstatus=True):
        ...
        if hvg==True:
            ...
            if adata.n_obs > 10 and adata.n_vars > 10:
                try:
                    ...
                    print(f"HVG calculation failed: {e}")
            else:
                print("Skipping HVG due to small data size")
    ...
    if adata.n_obs < 2 or adata.n_vars < 2:
        print(f"Warning: Not enough data after filtering ({adata.shape}). Skipping PCA.")
        # 더미 결과 반환 (이후 로직이 깨지지 않게)
        # 그냥 key_added를 임의로 추가해줌
        adata.obs['leiden_1_4'] = '0'
        adata.obs['louvain_1_4'] = '0'
        
        if default:
            return adata, total_clusters
        else:
            return adata
...
#    adata.X.fillna(0, inplace=True)
    ...
    # PCA 실행 시 n_comps 설정
    # 데이터 샘플 수가 적을 경우 n_comps가 샘플 수보다 크면 에러 발생
    n_comps = min(50, adata.shape[0] - 1, adata.shape[1] - 1)
    if n_comps < 1: n_comps = None # 너무 작으면 기본값 사용
        
    sc.pp.pca(adata, n_comps=n_comps)
    print(adata.shape)
    n_pcs_val = None if npc == 0 else npc
    sc.pp.neighbors(adata, n_neighbors=neigh, n_pcs=n_pcs_val)
